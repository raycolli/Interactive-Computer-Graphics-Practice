...
#define RandomFactor 2.0 //cannot be variable, because it is only use in
InitFountain(),
...
{
private:
GLfloat time; //How many steps the drop was "outside", when it falls into
the water, time is set back to 0
...
if (Position.y < 0.0)
{
/*the drop has fallen into the water. The problem is now, that we
cannot
set time to 0.0, because if there are more "DropsPerRay" than
"TimeNeeded" (See InitFountain())
...
SVertex * Vertices = new SVertex[NumOfVerticesStone*4]; //allocate mem for
...
for (i = 0; i<NumOfVerticesStone; i++)
{
Vertices[i + NumOfVerticesStone*3].x = cos(2.0 * PI /
NumOfVerticesStone * i) * InnerRadius;
Vertices[i + NumOfVerticesStone*3].y = 0.0; //Bottom
Vertices[i + NumOfVerticesStone*3].z = sin(2.0 * PI /
NumOfVerticesStone * i) * InnerRadius;
}
...
for (i = 0; i<NumOfVerticesStone; i++)
{
glTexCoord2f( 0.5+cos(i/GLfloat(NumOfVerticesStone)*360.0*PI/
180.0)/2.0,
...
glLoadIdentity(); //Load a new modelview matrix -> we can apply new
transformations
glTranslatef(0.0,0.0,-5.0);
//glRotatef(90.0,1.0,0.0,0.0); //Enable this line to look down on the
fountain
